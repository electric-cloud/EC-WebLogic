# -------------------------------------------------------------------------
  ...
  sub main() {
    ...
    $ecdaemonCmdLine .= '"system(\''. $::gScriptLocation;
    
    # if target: add to command string
    if($::gInstanceName && $::gInstanceName ne '') {
        $ecdaemonCmdLine .= ' ' . $::gInstanceName . ' ';
    }    
    ...
    if(%configuration){
        ...
        if($configuration{'weblogic_url'} ne ''){
            $ecdaemonCmdLine .= ' ' . $configuration{'weblogic_url'} . ' ';
        }
        
        if($configuration{'user'} ne ''){
            $ecdaemonCmdLine .= ' -Dweblogic.management.username=' . $configuration{'user'} . ' ';
        }
        
        if($configuration{'password'} ne ''){
            $ecdaemonCmdLine .= ' -Dweblogic.management.password=' . $configuration{'password'} . ' ';
        }
     
    }
    
    $ecdaemonCmdLine .= ' > WebLogicServer-$[/myJobStep/jobStepId].log 2>&1\')"';
 
    push(@args, $ecdaemonCmdLine);
 
    $props{'startManagedServerLine'} = createCommandLine(\@args);
    setProperties(\%props);
