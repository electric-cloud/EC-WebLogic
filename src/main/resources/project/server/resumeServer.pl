# -------------------------------------------------------------------------
  ...
  sub main() {
	...
	my $obtainedResult = resumeServer($::gServerName, $url, $user, $pass, $::gBlock, $::gWLSTAbsPath);
  ...
  sub resumeServer($){
      ...
      my ($serverName, $urlName, $user, $password, $block, $WLSTAbsPath)= @_;
...
try:\n
    ...
    state = \"NO_SERVER_FOUND\"\n";
      ...
      open (MYFILE, '>>resumeServer.jython');
      print MYFILE "$fileContent";
      close (MYFILE);
      
      push(@args, '"resumeServer.jython"');
     
      
      my $cmdLine = createCommandLine(\@args);
			
			
      $props{'resumeServerLine'} = $cmdLine;
      setProperties(\%props);
