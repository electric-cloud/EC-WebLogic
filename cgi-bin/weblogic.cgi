...
# esx.cgi -
#
# Get/set EC-WebLogic configuration info for UI
#
...
sub getCfgList {
    ...
    my $gcfg = new ElectricCommander::PropDB($::ec,"/projects/@PLUGIN_NAME@/weblogic_cfgs");
    ...
    foreach my $cfg (keys  %cfgs) {
        my $url = $gcfg->getCol("$cfg/weblogic_url");
        $xml .= "  <cfg>\n";
        $xml .= "     <name>$cfg</name>\n";
